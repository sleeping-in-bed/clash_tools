...
**wg_docker**
  ...
  Key highlights:
  - WireGuard config directory: ``/var/lib/clash_tools/wireguard/`` (override with ``CLASH_TOOLS_WG_CONFIG_DIR``)
  - Clash config directory: ``/var/lib/clash_tools/clash/`` (override with ``CLASH_TOOLS_CONFIG_DIR``)
